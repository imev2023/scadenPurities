...
def prediction(model_dir, 
    ...
    # Create output file names for individual predictions
    preds_256_file = out_name +"_m256"
    preds_512_file =out_name +"_m256"
    preds_1024_file = out_name +"_m256"

    # Save individual predictions
    preds_256.to_csv(preds_256_file, sep="\t")
    preds_512.to_csv(preds_512_file, sep="\t")
    preds_1024.to_csv(preds_1024_file, sep="\t")

    logger.info(f"[bold]Saved individual model predictions to:")
    logger.info(f" - [cyan]{preds_256_file}[/]")
    logger.info(f" - [cyan]{preds_512_file}[/]")
    logger.info(f" - [cyan]{preds_1024_file}[/]")
