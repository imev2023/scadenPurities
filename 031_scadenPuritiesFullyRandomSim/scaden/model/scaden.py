...
# tf.compat.v1.disable_eager_execution() # For error: Eager execution not enabled. 
import numpy as np
import pandas as pd
...
class Scaden(object):
    ...
    def __init__(
        ...
        model_dir,
        model_name,
        fprefix=None, # Needs to be defined in training but not prediction
        metric_dir=None, # Needs to be defined in training but not prediction 
        batch_size=128,
    ...
    def visualizationNew(self, logits, targets, classes):
        """
        Compute and return evaluation metrics as a dictionary
        """
        metrics = {
            "rmse": tf.sqrt(tf.reduce_mean(tf.square(logits - targets))).numpy(),
            "pcor": self.correlation_coefficient(logits, targets).numpy(),
            "accuracy01": self.compute_accuracy(logits, targets, 0.01).numpy(),
            "accuracy05": self.compute_accuracy(logits, targets, 0.05).numpy(),
            "accuracy1": self.compute_accuracy(logits, targets, 0.1).numpy(),
        }
        ...
        for i, cls in enumerate(classes):
            ...
            rel_error = tf.reduce_mean(tf.abs((y_true - y_pred) / (y_true + 1e-8))).numpy()
            pcor = self.correlation_coefficient(y_pred, y_true).numpy()

            metrics[f"mae_{cls}"] = mae
            metrics[f"mre_{cls}"] = rel_error
            metrics[f"pcor_{cls}"] = pcor
    ...
    def load_prediction_file(self, input_path, sig_genes, 
                             labels, 
                             scaling=None):
        # Returns a data np.array WITHOUT gene names. (n samples x n genes). Purity will be te last array if appliacble. 
        """
        ...
        # Collect genes to extract: sig_genes + optionally "purity". THis is included by default from scaden process. 
        all_genes_to_extract = list(sig_genes)
        # print(sig_genes[-1])

        # if self.purity_col in data.index:
        #     all_genes_to_extract.append(self.purity_col)

        # print(all_genes_to_extract[-1])

        # Subset to selected genes
        data = data.loc[[g for g in all_genes_to_extract if g in data.index]]
        ...
        # Separate out purity if present
        purity = None
        if self.purity_col in data.columns:
            purity = data[self.purity_col].to_numpy().reshape(-1, 1)
            # print(purity.shape)
            data = data.drop(columns=[self.purity_col])
            # print(type(data))
            # print(data.shape)

        # Apply scaling to signature gene columns
        # Scaling returns a np.ndarray (while data above is a pandas df. So we need to add back purity )
        if scaling:
            scaled_data = sample_scaling(data, scaling_option=scaling)

            # Reattach unscaled purity
            if purity is not None:
                data = np.hstack([scaled_data, purity])
        
        else: # If not scaling, ensure data is still converted from pd df to np.nd array like it is after scaling. 
            data = data.to_numpy()
    ...
    def train(self, 
        ...
        with progress_bar:
            ...
            for step in range(self.num_steps):
                ...
                # Periodic evaluation
                if step % eval_interval == 0 or step == self.num_steps - 1:
                      # or use a fixed val batch
                    metrics = self.visualizationNew(self.logits, y, self.labels)
                    metrics["step"] = step
                    all_metrics.append(metrics)
                    # logger.info(f"[Step {step}] RMSE: {metrics['rmse']:.4f} | "
                    #             f"Acc@5%: {metrics['accuracy05']:.4f} | PCor: {metrics['pcor']:.4f}")
        ...
        # print(f"Shape of metric df: {df.shape}")

        # # print(f"metric_dir type: {type(self.metric_dir)}, value: {self.metric_dir}")
        # print(f"Shape of metric dir: {self.metric_dir}")
        # print(f"Shape of fprefix: {self.fprefix}")
        # print(f"Shape of model name: {self.model_name}")

        df.to_csv(os.path.join(self.metric_dir,
                               self.fprefix+ "_"+ self.model_name+ "_metrics.csv"), index=True)
    ...
    def predict(self, input_path):
        ...
        # # If self.purity is not none, make sure the bulk data has a purity column. --> no longer possible, because rownames are removed during load_prediction_file()
        # if self.purity_col is not None:
        #     if self.purity_col not in self.data.columns:
        #         raise ValueError(f"Purity column '{self.purity_col}' not found in bulk data.")
        #     else:
        #         # Optionally: attach purity to the input features here if needed
        #         pass
